...
public class Lidar1D : MonoBehaviour
{
    [Header("Ray geometry")]
    [Tooltip("The 'true' measurement origin. If null, this object's transform is used.")]
    public Transform origin; // <-- Assign your 'LIDAR' (collider) object here
    ...
    public float sampleRateHz = 100f;    // 100 Hz as requested
    ...
    // -------- Outputs --------
    [NonSerialized] public float distance_m = Mathf.Infinity;    // noisy final
    ...
    public event Action<Lidar1D> OnSample;
    ...
    void FixedUpdate()
    {
        ...
        // 1. 'measurementSrc' is the TRUE sensor position (your LIDAR object with the collider).
        //    This is assigned via the public 'origin' field.
        Transform measurementSrc = origin ? origin : transform;

        // 2. 'raycastSrc' is THIS object (LIDAR_Sensor). We will move this
        //    just outside the drone's mesh.
        Transform raycastSrc = transform;

        // 3. The ray's DIRECTION is based on the TRUE sensor's orientation.
        Vector3 dirNominalWorld = measurementSrc.TransformDirection(localDirection).normalized;

        // 4. The ray's STARTING POSITION is from THIS object (outside the mesh).
        ...
        if (logToConsole && Time.timeAsDouble >= nextLogTime)
        {
            ...
            if (anyHit)
                Debug.Log($"[Lidar1D:{name}] hit {bestHit.collider.name} @ {distance_m:F3} m (raw {rawDistance_m:F3})");
            else
                Debug.Log($"[Lidar1D:{name}] no hit (> {maxRangeMeters:F2} m), reading={distance_m:F3} m");
        }
    }
